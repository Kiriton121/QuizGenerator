...
class AnswerCropper(tk.Tk):
    ...
    def on_canvas_drag(self, event):
        ...
        # 限制在图片范围内
        x1 = min(max(x1, self.img_left), self.img_left + self.show_img.width)
        y1 = min(max(y1, self.img_top), self.img_top + self.show_img.height)
        x0 = min(max(x0, self.img_left), self.img_left + self.show_img.width)
        y0 = min(max(y0, self.img_top), self.img_top + self.show_img.height)
    ...
    # ---------- 坐标换算：画布 -> 原始渲染像素 ----------
    def _canvas_box_to_original(self, box) -> Tuple[int,int,int,int] | None:
    ...
    def _crop_current_selection(self) -> Image.Image | None:
        if not self.sel_bbox_canvas:
    ...
    def on_finish(self):
        if not self.collected:
            if messagebox.askyesno("确认", "还没有保存任何选区，是否直接退出？"):
                self.destroy()
            return
        dest = OUTPUT_ROOT / self.stem
        dest.mkdir(parents=True, exist_ok=True)
        for qid in sorted(self.collected.keys(), key=lambda s: int(s[1:])):
            out = save_multipage_pdf(self.stem, qid, self.collected[qid], dest)
            print(f"[OK] {qid} -> {out}")
